...
%         paras.ASR_result = ori_struct;
        ...
        paras.ASR_result(j).cutoff = thresASR(j);
        % run ICA
        EEG_asr = pop_runica(EEG_asr, 'icatype', 'runica', 'extended',1);
        [w_asr, s_asr] = deal(EEG_asr.icaweights, EEG_asr.icasphere);
        paras.ASR_result(j).icaweights = w_asr;
        paras.ASR_result(j).icasphere = s_asr;
        % spatial_filter ASR ICs
        spatial_filter_ASR = w_asr * s_asr;

        % data modified
        paras.ASR_result(j).whole_Section.dataModified = nnz(EEG_asr.data - EEG_resampled.data)/numel(EEG_resampled.data);
        paras.ASR_result(j).clean_Section.dataModified = nnz(EEG_asr.data(:,mask) - EEG_clean.data)/numel(EEG_clean.data);

        % compute var for each channels
        paras.ASR_result(j).whole_Section.var = var(EEG_asr.data,0,2);
        paras.ASR_result(j).clean_Section.var = var(EEG_asr.data(:,mask),0,2);

        % compute raw ICs activities
        paras.ASR_result(j).whole_Section.ICact_ori = var(spatial_filter_whole*EEG_asr.data,0,2);
        paras.ASR_result(j).clean_Section.ICact_ori = var(spatial_filter_whole*EEG_asr.data(:,mask),0,2);

        % compute clean ICs activites
        paras.ASR_result(j).whole_Section.ICact_ori_clean = var(spatial_filter_clean*EEG_asr.data,0,2);
        paras.ASR_result(j).clean_Section.ICact_ori_clean = var(spatial_filter_clean*EEG_asr.data(:,mask),0,2);

        % compute ASR ICs activites
        paras.ASR_result(j).whole_Section.ICact_asr = var(spatial_filter_ASR*EEG_asr.data,0,2);
        paras.ASR_result(j).clean_Section.ICact_asr = var(spatial_filter_ASR*EEG_asr.data(:,mask),0,2);
